...
export function ObservabilityDashboard() {
  ...
  const routeCounts = useMemo(() => {
    ...
    for (const log of logs) map.set(log.route, (map.get(log.route) ?? 0) + 1);
    return [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5);
  }, [logs]);

  const consumerCounts = useMemo(() => {
    const map = new Map<string, number>();
    for (const log of logs) map.set(log.consumer, (map.get(log.consumer) ?? 0) + 1);
    return [...map.entries()].sort((a, b) => b[1] - a[1]).slice(0, 5);
  }, [logs]);
  ...
  return (
    <Box>
      <Flex justifyContent="space-between" alignItems="center" marginBottom="spacingL">
        <Box>
          <Heading marginBottom="none">CDA Observability</Heading>
